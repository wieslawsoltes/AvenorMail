...
export async function createApplication(options={}){
 ...
 async function ingest(user,accountId){const messages=await providers.syncAccount(user,accountId),scope='user:'+user.userId;for(const m of messages){const existing=db.prepare('SELECT * FROM records WHERE id=?').get(m.id);if(existing&&existing.scope!==scope)throw error('Provider identifier scope conflict',409);if(m.deleted){if(existing)db.prepare('UPDATE records SET data=?,deleted=1,version=version+1,updated=? WHERE id=?').run(JSON.stringify({...JSON.parse(existing.data),providerSyncStamp:writeback.stamp()}),Date.now(),m.id);continue;}const attachments=[];for(let i=0;i<(m.attachments||[]).length;i++){const a=m.attachments[i],id=digest(m.id+':'+i+':'+a.name);const bytes=a.bytes||a.content;if(!bytes)continue;await bucket.put(id,bytes);db.prepare('INSERT INTO files(id,owner,scope,name,type,size,created) VALUES (?,?,?,?,?,?,?) ON CONFLICT(id) DO NOTHING').run(id,user.userId,scope,a.name||'attachment',a.type||'application/octet-stream',bytes.byteLength,Date.now());attachments.push({id,name:a.name||'attachment',type:a.type||'application/octet-stream',size:bytes.byteLength});}const data=writeback.preservePending(m.id,{...m,attachments,accountId,sample:false,providerSyncStamp:writeback.stamp()});delete data.id;delete data.deleted;db.prepare('INSERT INTO records(id,owner,scope,kind,data,version,updated,deleted) VALUES (?,?,?,\'message\',?,1,?,0) ON CONFLICT(id) DO UPDATE SET data=excluded.data,version=records.version+1,updated=excluded.updated,deleted=CASE WHEN records.deleted=1 AND EXISTS(SELECT 1 FROM jobs WHERE type=\'provider_mutation\' AND json_extract(payload,\'$.recordId\')=records.id AND status NOT IN (\'completed\',\'accepted\',\'cancelled\')) THEN 1 ELSE 0 END').run(m.id,user.userId,scope,JSON.stringify(data),Date.now());}await providers.acknowledgeSync?.(user,accountId,messages.batchId);audit.append(user.userId,'provider.sync',scope,{accountId,count:messages.length});emit(scope,{type:'record-change'});return {count:messages.length};}
 async function externalSend(payload,{user,idempotencyKey,markAccepted}={}){
  ...
  if(accountId)for(const a of message.attachments||[]){const file=db.prepare('SELECT * FROM files WHERE id=? AND scope=?').get(a.id,row.scope);if(!file)throw error('Attachment access denied',403);const obj=await bucket.get(a.id);if(!obj)throw error('Attachment missing',409);a.base64=Buffer.from(await obj.arrayBuffer()).toString('base64');}
  await permissions.require({user,scope:row.scope,recordId:row.id,action:'send'});
  ...
  db.exec('BEGIN IMMEDIATE');try{
   const collaborative=collaborationDocumentHTML(db,row.id);if(collaborative!=null)message.body=collaborative;
 ...
 const scheduler=new DurableScheduler({db,authorize,emit,allowedOrigins:origins,handlers:{send:(payload,ctx)=>externalSend(payload,{user:payload.user,idempotencyKey:ctx.idempotencyKey,markAccepted:ctx.markAccepted}),provider_sync:async payload=>{const user=auth.user(payload.userId);if(user)await ingest(user,payload.accountId);return {status:'completed'};}}});scheduler.migrate();
 const writeback=installMailWriteback({db,providers,auth,scheduler,emit});
 const invitations=new InvitationService({db,scheduler,authorize,emit,env,resolveAccount:async({user,accountId})=>{const accounts=await providers.listAccounts(user);const a=(Array.isArray(accounts)?accounts:accounts.accounts).find(a=>a.id===accountId);if(!a)throw error('Choose a connected mail account',400);return a;},deliver:({user,accountId,to,subject,text,raw,idempotencyKey})=>providers.sendMail({user,accountId,message:{to,subject,body:text},mime:raw,idempotencyKey})});invitations.migrate();
 hub=new RealtimeHub({db,emit:event=>emit(event.scope,{type:'record-change',recordId:event.recordId}),authenticate:req=>auth.authenticate(req),authorize:async args=>{if(args.recordId&&args.action==='write'){const r=db.prepare('SELECT kind,data FROM records WHERE id=? AND deleted=0').get(args.recordId);if(!r||r.kind!=='message'||JSON.parse(r.data).folder!=='drafts'||JSON.parse(r.data).delivery==='submitting')return false;}return authorize(args);},allowedOrigins:origins});hub.migrate();
 ...
 if(path==='/api/health')return response({ok:true,version:'0.2.0',scheduler:!!scheduler.timer});
 ...
 if(key&&path==='/api/record'&&['POST','PATCH','DELETE'].includes(request.method)){
  ...
  const user=await auth.authenticate(request);if(user){const body=await request.clone().text(),signature=digest(request.method+path+body),prior=db.prepare('SELECT * FROM api_operations WHERE owner=? AND key=?').get(user.userId,key),opKey=user.userId+':'+key;
 ...
 }catch(e){if(!e.status)console.error('API request failed',e);result=response({error:e.status?e.message:'Server request failed; your pending changes have been preserved'},e.status||500);}finally{if(operation)pendingOperations.delete(operation.opKey);}
 ...
 async function retention(){for(const policy of db.prepare('SELECT * FROM policies WHERE retention_days>0 AND legal_hold=0').all()){const ids=db.prepare('SELECT id FROM records WHERE scope=? AND deleted=0 AND updated<?').all(policy.scope,Date.now()-policy.retention_days*86400000);for(const row of ids)db.prepare('UPDATE records SET deleted=1,version=version+1,updated=? WHERE id=?').run(Date.now(),row.id);if(ids.length){audit.append('system','retention.applied',policy.scope,{count:ids.length});emit(policy.scope,{type:'record-change'});}}}
 let providerTimer,retentionTimer;function startWorkers(){scheduler.start();providerTimer=setInterval(async()=>{for(const row of db.prepare('SELECT user_id FROM auth_accounts WHERE disabled=0').all()){const user=auth.user(row.user_id);if(!user)continue;try{const accounts=await providers.listAccounts(user);for(const a of Array.isArray(accounts)?accounts:accounts.accounts||[])scheduler.enqueue({id:'sync:'+a.id+':'+Math.floor(Date.now()/60000),type:'provider_sync',payload:{userId:user.userId,accountId:a.id},runAt:Date.now(),owner:user.userId,scope:'user:'+user.userId});}catch(e){console.error('Sync schedule failed',e.message);}}},60000);providerTimer.unref();retentionTimer=setInterval(()=>retention().catch(console.error),3600000);retentionTimer.unref();}
 ...
 hub.attach(server);return {db,env,auth,permissions,providers,scheduler,invitations,hub,audit,handle,server,startWorkers,ingest,externalSend,async close(){scheduler.stop();clearInterval(providerTimer);clearInterval(retentionTimer);await hub.close();await new Promise(resolve=>server.listening?server.close(resolve):resolve());while(scheduler.running)await new Promise(resolve=>setTimeout(resolve,25));db.close();}};
}
