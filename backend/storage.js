...
export class FileBucket {
 ...
 async put(id,data){const bytes=Buffer.from(data instanceof ReadableStream?await new Response(data).arrayBuffer():data);let output=bytes;if(this.key){const iv=Buffer.from(crypto.getRandomValues(new Uint8Array(12))),cipher=createCipheriv('aes-256-gcm',this.key,iv);output=Buffer.concat([Buffer.from('AVN1'),iv,cipher.update(bytes),cipher.final(),cipher.getAuthTag()]);}const tmp=this.path(id)+'.'+randomUUID()+'.tmp';writeFileSync(tmp,output,{mode:0o600});renameSync(tmp,this.path(id));}
 async get(id){if(!existsSync(this.path(id)))return null;let bytes=readFileSync(this.path(id));if(bytes.subarray(0,4).toString()==='AVN1'){if(!this.key)throw Error('DATA_KEY is required to read encrypted attachments');const decipher=createDecipheriv('aes-256-gcm',this.key,bytes.subarray(4,16));decipher.setAuthTag(bytes.subarray(-16));bytes=Buffer.concat([decipher.update(bytes.subarray(16,-16)),decipher.final()]);}return {body:new Blob([bytes]).stream(),arrayBuffer:async()=>bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength)};}
 async head(id){const obj=await this.get(id);return obj?{size:(await obj.arrayBuffer()).byteLength}:null;}
 async delete(id){try{unlinkSync(this.path(id));}catch(error){if(error.code!=='ENOENT')throw error;}}
}
