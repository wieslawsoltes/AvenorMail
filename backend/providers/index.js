...
export class ProviderService {
  ...
  listAccounts(user) {
    ...
    return this.db.prepare('SELECT * FROM provider_accounts WHERE owner=? ORDER BY created_at,id').all(owner).map(publicAccount);
  }
  ...
  async api(account, url, init = {}, raw = false) {
    ...
    const expected = account.provider === 'microsoft' ? 'graph.microsoft.com' : 'gmail.googleapis.com';
    if (target.protocol !== 'https:' || target.hostname !== expected || (target.port && target.port !== '443') || target.username || target.password) fail('The provider returned an unsafe continuation URL.', 502, 'provider_invalid_url');
    const token = await this.accessToken(account);
    const call = accessToken => (raw ? fetchRaw : fetchJSON)(this.fetchImpl, target.toString(), { ...init, headers: { ...init.headers, Authorization: `Bearer ${accessToken}` } });
  ...
  async syncAccount(user, id) {
    ...
    const task = (async () => {
      ...
      const previous = JSON.parse(account.cursor_json || '{}');
      ...
      try {
        if (account.provider === 'smtp') result = await syncImap(unseal(account.encrypted_secret, this.env, accountContext(account)), previous, this.env);
          ...
          result = account.provider === 'microsoft' ? await graphSync(api, (url, init) => this.api(account, url, init, true), previous, limit) : await gmailSync(api, previous, limit);
        }
      } catch (error) {
      ...
      const messages = result.messages.flatMap(m => {
        const alias = this.db.prepare('SELECT * FROM provider_message_aliases WHERE owner=? AND account_id=? AND alias=?').get(account.owner,account.id,String(m.providerId));
        // A move produces a source-folder removal. Do not delete the stable
        // local record merely because the old location disappeared.
        if (m.deleted && alias && (alias.canonical !== m.providerId || m.folder && alias.folder && m.folder !== alias.folder)) return [];
        return [{ ...m, id: alias?.record_id || `provider:${account.id}:${hash(String(m.providerId)).slice(0,32)}`, accountId: account.id, provider: account.provider, sample: false }];
      });
  ...
  async sendMail({ user, accountId, message, mime, idempotencyKey }) {
  ...
  async updateMessage(user, accountId, providerId, inputPatch, { idempotencyKey } = {}) {
    ...
    const task = before.catch(() => {}).then(() => this.performMessageMutation(account, providerId, patch, idempotencyKey));
  ...
  async performMessageMutation(account, originalId, patch, key) {
    ...
    if (account.provider !== 'smtp') {
      ...
      const fresh = this.account({ userId: account.owner },account.id);
      ...
      const allowed = account.provider === 'google' ? scopes.includes('https://www.googleapis.com/auth/gmail.modify') || scopes.includes('https://mail.google.com/') : scopes.some(scope => scope === 'mail.readwrite' || scope.endsWith('/mail.readwrite'));
  ...
  async handle(request, user) {
    ...
    if (path !== '/api/accounts' && !oauth && !item) return null;
    ...
    if (item && !item[2] && request.method === 'DELETE') {
      ...
      if (this.syncLocks.has(account.id) || this.tokenLocks.has(account.id) || this.mutationQueues.has(account.id)) fail('This mailbox is busy. Retry after the current operation.', 409);
      this.db.prepare('DELETE FROM provider_sync_batches WHERE owner=? AND account_id=?').run(account.owner, account.id);
      this.db.prepare('DELETE FROM provider_accounts WHERE owner=? AND id=?').run(account.owner, account.id);
