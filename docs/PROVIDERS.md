...
| `PROVIDER_SYNC_LIMIT` | Optional | Default `100`. Cloud sync bounds the requested page size to 6–500, distributing the Microsoft page budget over accessible standard folders. IMAP uses this as its maximum examined UIDs per batch. |
...
| `PROVIDER_IMAP_MAX_UIDS` | Optional, IMAP | Default `250000` UID memberships per complete reconciliation snapshot. Exceeding the limit fails without advancing synchronization. |
| `PROVIDER_IMAP_FOLDER_BATCH_LIMIT` | Optional, IMAP | Default `25` mailbox membership lists per durable snapshot-building page. |
| `PROVIDER_LEASE_MS` | Optional | Default `90000`, constrained to 30000–300000 ms. Shared-database account lease duration, renewed while active. |
| `PROVIDER_LEASE_WAIT_MS` | Optional | Default `10000`, maximum 60000 ms. Contention wait before a retryable busy response. |
...
- IMAP reconciles all selectable mailboxes, including flags, expunges, folder removals, unique exact-content moves, and UIDVALIDITY changes through durable bounded snapshots. See [MAILBOX-SYNC.md](MAILBOX-SYNC.md) for copy/move identity rules, pagination, and scale limits. SMTP sends are not appended to IMAP Sent automatically; providers that do not save submitted mail rely on Avenor's own sent record.
...
- Imported read/flag/folder changes can be written back through `updateMessage` when the root backend has durably queued them. Body edits, new provider draft creation, provider folder creation, and permanent deletion are outside this mutation API. Discovered custom IMAP folders support exact-path moves; externally changed flags and message locations are reconciled.
...
## Shared Exchange mailboxes and multi-process coordination

[MAILBOX-SYNC.md](MAILBOX-SYNC.md) documents directory search with granted permissions, explicit address attachment, real folder-access verification, mailbox-scoped synchronization and submission, and provider-enforced Send As/Send on Behalf permissions. Microsoft Graph cannot enumerate every mailbox permission held by a user.

A database lease serializes mailbox sync, provider mutations, submissions, direct calendar/contact cloud requests, configuration changes, and disconnect across backend instances. Credential refresh uses a separate lease keyed to the primary OAuth account so child shared mailboxes reuse serialized token rotation without nested deadlock. Leases renew while active; expired owners are fenced before requests and before durable results. Unknown sends retain their existing no-automatic-retry semantics. All nodes must share the authoritative database; copying separate SQLite files does not create a cluster.
